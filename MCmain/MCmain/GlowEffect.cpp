...
HRESULT GlowEffect::Initialize(ID3D11Device* device, ID3D11DeviceContext* context)
{
	HRESULT hr = S_OK;
...
HRESULT GlowEffect::UseEffect(ID3D11Device* device, ID3D11DeviceContext* context, 
	ID3D11DepthStencilView* depthstencil, ID3D11ShaderResourceView* tSRV)
{
	...
	SHADERMANAGER.UseShader(device, context, "glow_color_select");
	...
	colorBuffer->SetRenderTarget(device, context, depthstencil);
	colorBuffer->ClearRenderTarget(device, context, depthstencil);
	SHADERMANAGER.UseShader(device, context, "glow_final");
	context->PSSetShaderResources(0, 1, &tSRV);
...
HRESULT GlowEffect::Draw(ID3D11Device* device, ID3D11DeviceContext* context)
{
	return S_OK;
}

ID3D11ShaderResourceView*& GlowEffect::GetShaderResourceView()
{
