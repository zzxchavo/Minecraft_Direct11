...
ID3D11RasterizerState*  raState = NULL;
D3D11_VIEWPORT vp;
...
HRESULT InitWindow( HINSTANCE hInstance, int nCmdShow )
{
    ...
    g_hWnd = CreateWindow( L"WindowClass", L"MyMineCraft", WS_OVERLAPPEDWINDOW,
...
HRESULT InitDevice()
{
    ...
    if( FAILED( hr ) )
	{
		MessageBox(NULL,L"CreateDepthStencilView Failed",L"Error - System",MB_OK);
        return hr;
	}
	...
	hr = g_pd3dDevice->CreateRasterizerState(&raDesc,&raState);
	if(FAILED(hr))
	{
		MessageBox(NULL,L"CreateRasterizerState Failed!",L"Error",MB_OK);
		return false;
	}
...
LRESULT CALLBACK WndProc( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
{
    ...
    switch( message )
    {
		...
		case WM_KEYDOWN:
			m_world.GetInputClass()->SetKeyState(wParam,true);
			break;
...
//RenderFrame Operation
	m_world.RenderFrame(g_pRenderTargetView,
		g_pDepthStencilView,
		vp,
		g_pd3dDevice,
		g_pImmediateContext);
