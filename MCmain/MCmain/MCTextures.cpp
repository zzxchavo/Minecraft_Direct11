...
DefinesAndTextures* DefinesAndTextures::instance = nullptr;

DefinesAndTextures*& DefinesAndTextures::GetInstance()
{
	if (instance == nullptr)
	{
		instance = new DefinesAndTextures;
	}
	return instance;
}
...
bool DefinesAndTextures::Initialize(ID3D11Device *device,ID3D11DeviceContext *context)
{
	...
	hr = m_texs[GUI_BORDER_LEFT	]->Initialize(device, L"GUI/border.png"); if (FAILED(hr))return false;
	hr = m_texs[GUI_BORDER_RIGHT]->Initialize(device, L"GUI/border.png"); if (FAILED(hr)) return false;
	hr = m_texs[GUI_BORDER_MID	]->Initialize(device, L"GUI/border.png"); if (FAILED(hr))return false;
	return true;
}	
